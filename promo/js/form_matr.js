...
$('form[id^="mform1_"]').submit(function(event) {
    ...
    var ano_id=$('select[name="materias_id"]').val();
    var userid=$('input[name^="userid_"]').length;
    var promo_id=$('select[name="promo_id"]').val();

    
    
    if(!ano_id ){      
            alert("¡Debe seleccionar un año a matricular los estudiantes!");
            return false;  
    }
    if(!promo_id && !userid){      
        alert("¡Debe seleccionar una promocion/usuario para inscribir en el curso!");
        return false;  
    }
